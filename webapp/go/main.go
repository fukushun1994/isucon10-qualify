...
var dbEstate, dbChair *sqlx.DB
var connEstate, connChair *MySQLConnectionEnv
...
func NewMySQLConnectionEnv() (estate *MySQLConnectionEnv, chair *MySQLConnectionEnv) {
...
func main() {
	...
	connEstate, connChair = NewMySQLConnectionEnv()

	var err error
	dbEstate, err = connEstate.ConnectDB()
	if err != nil {
	...
	dbChair, err = connChair.ConnectDB()
	if err != nil {
		e.Logger.Fatalf("DB connection failed : %v", err)
	}
...
func initialize(c echo.Context) error {
	...
	// for estate
	if err := runSQL(c, filepath.Join(sqlDir, "0_Schema.sql"), connEstate); err != nil {
		return err
	}
	if err := runSQL(c, filepath.Join(sqlDir, "1_DummyEstateData.sql"), connEstate); err != nil {
		return err
	}

	// for chair
	if err := runSQL(c, filepath.Join(sqlDir, "0_Schema.sql"), connChair); err != nil {
		return err
	}
	if err := runSQL(c, filepath.Join(sqlDir, "2_DummyChairData.sql"), connChair); err != nil {
		return err
	}
...
func runSQL(c echo.Context, path string, conn *MySQLConnectionEnv) error {
...
func postChair(c echo.Context) error {
	...
	query := fmt.Sprintf("INSERT INTO chair(id, name, description, thumbnail, price, height, width, depth, color, features, kind, popularity, stock) VALUES %s", strings.Join(values, ","))
	_, err = tx.Exec(query)
	if err != nil {
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	chair := Chair{}
	query := `SELECT * FROM chair WHERE id = ?`
	err = dbEstate.Get(&chair, query, id)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	err = dbEstate.Select(&estates, query, long, short, Limit)
	if err != nil {
		if err == sql.ErrNoRows {
			return c.JSON(http.StatusOK, EstateListResponse{[]Estate{}})
		}
...
func searchEstateNazotte(c echo.Context) error {
	...
	err = dbEstate.Select(&estatesInBoundingBox, query)
	if err == sql.ErrNoRows {
